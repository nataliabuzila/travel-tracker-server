...
router.post('/', fileUploader.single('imageURL'), (req, res, next) => {
    ...
    Review.create({title, description, date, publicOrPrivate, imageURL: req.file.path, trip })
    .then ((newReview) => {
        console.log(newReview)
        return Trip.findByIdAndUpdate(trip, { $push: {reviews: newReview._id}}, {new: true})
...
router.put ('/:reviewId', fileUploader.single('imageURL'), (req, res, next) =>{
    const {reviewId} = req.params;
    ...
    Review.findByIdAndUpdate(reviewId, {...req.body, imageURL: imageUrl }, {new: true})
